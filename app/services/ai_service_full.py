...
def save_analytics(db: Session, target_date: date, prediction_result: dict, df: pd.DataFrame) -> Analytics:
    ...
    # features와 variable_scores에 같은 값 (정형 데이터)
    structured_features = {}
    ...
    if existing:
        ...
        existing.variable_scores = structured_features
        db.commit()
        db.refresh(existing)
        return existing
    ...
    db_analytics = Analytics(
        ...
        variable_scores=structured_features
    )
...
def save_contents(db: Session, news_list: list) -> list:
    ...
    for news in news_list:
        # relation_nation 첫 번째 국가로 region 찾기
        region_id = None
        relation_nations = news.get("relation_nation", [])
        if relation_nations:
            country_name = relation_nations[0]
...
def save_regions(db: Session, news_list: list):
    ...
    # region_score 계산 (해당 국가 뉴스의 source_score 절대값 평균)
    for country in countries:
        region = db.query(Region).filter(Region.name == country).first()
        if region:
            contents = db.query(Content).filter(Content.region_id == region.id).all()
            if contents:
                scores = [abs(float(c.source_score)) for c in contents if c.source_score]
                region.region_score = sum(scores) / len(scores) if scores else 0.0
