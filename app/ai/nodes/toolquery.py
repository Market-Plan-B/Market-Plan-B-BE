...
- 단, 유저가 원하는 출력 형식(표/리스트/나열/문단)을 최대한 그대로 따른다.
...
2) "리스트", "bullet", "나열", "정리해줘" 등의 표현이 포함되면
   ...
   - 예:
     ...
     - news_rag: OOO 관련 뉴스가 다수, 날짜/클러스터 중심 요약
...
- news_rag_result (list 예상)
  - 벡터DB에서 찾은 뉴스 리스트.
  - 각 항목에는 보통:
      - document: 뉴스 본문/요약 텍스트
      - metadata.cluster_id: 해당 뉴스의 클러스터 ID
      - metadata.published: 뉴스 날짜(YYYY-MM-DD)
    등이 포함될 수 있다.
