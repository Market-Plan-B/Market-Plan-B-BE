...
- 목적:
    - VDB(벡터DB)에 저장된 뉴스 임베딩(summary_embedding)을 사용해
      유저가 제공한 문장/뉴스와 유사한 뉴스들을 검색하거나,
      특정 클러스터/기간의 주요 뉴스들을 검색한다.
- 실제 호출 시 input 형식:
{
  "query": "유저가 준 뉴스 문장 또는 키워드",   # 필수
  "top_k": 5,                                   # 선택, 기본 5
  "cluster_id": "cluster_2",                    # 선택
...
- (옵션) news_rag:
    - 관련 뉴스 흐름이 필요할 때, 지표 변화와 연관된 뉴스 클러스터/주요 이벤트를 함께 참고하는 용도.
...
- (옵션) news_rag:
    - 가격 급등락 관련 주요 이벤트/뉴스를 함께 가져와서,
      정형 지표 변화와 사건·뉴스 흐름을 같이 설명할 때 사용.
...
- (옵션) news_rag:
    - 같은 클러스터 내 대표 뉴스들을 함께 가져와,
      정형 패턴과 비정형 뉴스 내용을 함께 비교·참고하는 용도.
...
2. 최소 0개 이상, 보통 1~3개 정도의 툴 단계(indicator_snapshot, pattern_lookup, news_rag)를 포함할 수 있다.
...
5) 위 조건들이 모두 해당하지 않으면:
   - 목적 유형과 질문 내용에 따라 indicator_snapshot, pattern_lookup, news_rag 중
